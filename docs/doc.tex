...
\fancyhead[LO]{\texttt{Mazluc}} % Left on Odd
\fancyhead[RO, LE]{\thepage}                                               % Right on Odd
\fancyhead[RE]{\nouppercase{\leftmark}}
\fancyfoot[LO, RE]{\footnotesize \textit{Converged Engineering Homelab}}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\footrulewidth}{0.4pt}
\fancypagestyle{plain}{
    \fancyhf{} % Clear defaults
    \fancyhead[LO]{\texttt{Mazluc}}
...
The grounds for this project are the need for a powerful, flexible, and efficient \gls{homelab} that can handle various
...
\midrule
\textbf{Total} &  & \textbf{3568.—} & \\
\bottomrule
\end{tabular}
    \caption{Rack Components \cite{galaxus}}
\end{table}
...
\toprule
\textbf{ID} & \textbf{Description} & \textbf{Notes} \\ \midrule
\texttt{R-00} & Need an hypervisor to host multiple VMs & Proxmox\\
\texttt{R-01} & Need a reverse proxy to route to the right service securely (\texttt{gitea.local}, ecc...) & nginx\\
\texttt{R-02} & Need a container manager to provision/manage/destroy isolated dev containers & docker, lazydocker\\
\texttt{R-03} & Need a version control service to manage repositories \& version control & Gitea, tea\\
\texttt{R-04} & Need a CI/CD Runner service to test code, package docker images on triggers & Gitea actions\\
\texttt{R-05} & Need an image storage local to the server, to pull images quickly and securely & Docker registry\\
\texttt{R-06} & Need a docker container to host a window manager to be accessed via browser  & webtop\\
\bottomrule
\end{tabular}
...
\blankpage
\printbibliography[heading=bibintoc, title=Sitography]
